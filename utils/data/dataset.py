...
def create_adjacency_list(edges, n_edge_types):
    a = [[] for _ in range(n_edge_types)]
    for edge in edges:
        src_idx = edge[0]
        e_type = edge[1]
        tgt_idx = edge[2]
        a[e_type-1].append([src_idx, tgt_idx])
    return a
...
class bAbIDataset():
    ...
    def __getitem__(self, index):
        am = create_adjacency_list(
            self.data[index][0], self.n_edge_types)
